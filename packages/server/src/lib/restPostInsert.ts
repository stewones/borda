...
import {
  ...
  getCloudTrigger,
} from '../lib_elegante/Cloud'; // @todo rethink this with the Borda instance
...
export async function restPostInsert({
  ...
  try {
    ...
    return res
      .status(500)
      .json(new BordaError(ErrorCode.REST_POST_ERROR, err as object).toJSON());
  }
}

export async function serverPostInsert<TSchema = Document>({
  docQRL,
  inspect,
  request,
}: {
